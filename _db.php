...
function dbi_o() {
	...
	global $dbi_link;
	if ($dbi_link===false) {
		$dbi_link = new mysqli($GLOBALS['DBHost'], $GLOBALS['DBUser'], $GLOBALS['DBPass'], $GLOBALS['DBName']);					//  Connexion à MySQL
		if ($dbi_link===false) {
			if ((include 'inc_down.html')===false) {
				print('<h1>Down for maintenance</h1><p>This website is currently down for maintenance. We are currently working on it, so please come back in a few hours...</p><hr/>'.$_SERVER['HTTP_HOST'].'<i>');
			}
...
// === Effectue une RECHERCHE dans la base de données du site OLD FUNCTION================================================================================================================
function mq_o() {
	// MySQL 4.1 et supérieur
	$link = mysql_connect($GLOBALS['DBHost'], $GLOBALS['DBUser'], $GLOBALS['DBPass']);					//  Connexion à MySQL
	if (!$link) { include('inc_down.html'); exit(); }						//  En cas d'erreur de connexion, on arrête tout !
	mysql_select_db($GLOBALS['DBName'], $link);															// Sélectionne la base de données
...
// === EXECUTE la requête passée en paramètre OLD FUNCTION=================================================================================================================================
function mq_x($request, $do_log=true) {
	$link = mq_o();
	$result = mysql_query($request, $link);
	if ($do_log && substr($request, 0, 7) != 'SELECT ' && mysql_affected_rows($link)>0) { db_log($request); }

	if (mysql_errno($link)) {
		print('<script type="text/javascript">alert(\'MySQL error '.mysql_errno($link).': '.addslashes(mysql_error($link)).'\nWhen executing :\n'.addslashes($request).'\n\');</script>');
	}
	else {
		return $result;
	}
